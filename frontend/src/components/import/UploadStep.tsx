...
export function UploadStep({ file, loading, onFileSelect, onUpload }: UploadStepProps) {
  return (
    <section className="card">
      <h2 className="text-lg font-medium text-white">1) Upload CSV or XLSX file</h2>
      ...
      <div className="mt-4 rounded-lg border border-dashed border-white/10 p-5">
        <input
          type="file"
          accept=".csv,.xlsx"
        ...
        />
        <p className="mt-2 text-sm text-slate-500">{file ? `Selected: ${file.name}` : 'No file selected'}</p>
      </div>
      <div className="mt-4">
