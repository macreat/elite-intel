...
export function MappingStep({ columnsDetected, mapping, loading, onMappingChange, onSubmit }: MappingStepProps) {
  return (
    <section className="card">
      <h2 className="text-lg font-medium text-white">2) Map columns</h2>
      <p className="mt-1 text-sm text-slate-400">Review and adjust file-to-system field mappings.</p>

      <div className="mt-4 grid gap-3">
        {SYSTEM_FIELDS.map((field) => (
          <div key={field} className="grid gap-2 md:grid-cols-2 md:items-center">
            <label className="text-sm font-medium capitalize text-slate-300">{field.replace('_', ' ')}</label>
            <select
              className="field"
              value={mapping[field] ?? ''}
